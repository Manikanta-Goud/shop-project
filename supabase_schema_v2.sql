...
-- 3. WISHLIST TABLE (Persists "Likes")
...
-- 5. ORDERS TABLE (For Recent Orders)
...
-- Capture full name, email and metadata during signup
...
-- Policies for public viewing of products
create policy "Products are viewable by everyone" on products for select using (true);

-- Policies for profile (user can only see/edit their own)
create policy "Users can view own profile" on profiles for select using (auth.uid() = id);
create policy "Users can update own profile" on profiles for update using (auth.uid() = id);

-- Policies for wishlist (user can only see/manage their own)
create policy "Users can manage own wishlist" on wishlist for all using (auth.uid() = user_id);

-- Policies for cart (user can only see/manage their own)
